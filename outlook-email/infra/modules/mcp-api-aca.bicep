...
@description('The HTTPS URL of the ACA container app hosting the MCP endpoints, e.g. https://<fqdn>')
param acaServiceUrl string
...
// Create the MCP API definition in APIM pointing to the ACA backend
resource mcpApi 'Microsoft.ApiManagement/service/apis@2023-05-01-preview' = {
  parent: apimService
  name: 'mcp'
  properties: {
    displayName: 'MCP API'
    description: 'Model Context Protocol API endpoints (ACA backend)'
    subscriptionRequired: false
    path: '/'
    protocols: [
      'https'
    ]
    serviceUrl: '${acaServiceUrl}/'
  }
}

// Apply ACA-specific policy at the API level (no function host key, buffer-response disabled)
resource mcpApiPolicy 'Microsoft.ApiManagement/service/apis/policies@2023-05-01-preview' = {
  parent: mcpApi
  name: 'policy'
  properties: {
    format: 'rawxml'
    value: loadTextContent('mcp-api-aca.policy.xml')
  }
  dependsOn: [
